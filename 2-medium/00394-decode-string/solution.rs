    ...
    pub fn decode_string(s: String) -> String {
        ...
        for byte in s.into_bytes() {
            ...
            if byte.is_ascii_digit() {
                ...
                if let (Some(last_multiplier), Some(last_strbyte)) = (stack_multipliers.pop(), stack_strbytes.pop()) {
                    let mut tmp: Vec<u8> = Vec::new();
                    for _ in 0..last_multiplier {
                        tmp.extend_from_slice(&current_strbyte);
                    }
                    current_strbyte = last_strbyte.clone();
                    current_strbyte.extend(tmp);
                }
            }
        }
